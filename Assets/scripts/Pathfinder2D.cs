...
public class Pathfinder2D : MonoBehaviour 
{
	...
	private void Create2DMap()
	{
		...
		for (int i = 0; i < height; i++)
		{
			for (int j = 0; j < width; j++)
			{
				...
				{
					//UnityEngine.Debug.Log (h.transform.gameObject.name);
	...
	{
		//UnityEngine.Debug.Log ("Set start and end node");
	...
	{
		//UnityEngine.Debug.Log ("Find Closest node");
		...
		Node n = Map[x, y];
		//UnityEngine.Debug.Log ("checking node n at " + n.x + " " + n.y);
		if (n.walkable)
		{
		...
		else
		{
			...
			//UnityEngine.Debug.Log ("returning null for closest node");
			return null;
		}
	}
